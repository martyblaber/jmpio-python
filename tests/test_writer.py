...
def test_round_trip_datetime():
    ...
    df = pd.DataFrame({
        ...
        'datetimes': [datetime(2020, 1, 1, 12, 0), datetime(2020, 1, 2, 13, 30), None, datetime(2020, 1, 4, 18, 15)],
        'durations': [timedelta(hours=1), timedelta(minutes=30), None, timedelta(days=2)]
    })
    ...
    try:
        ...
        for i in [0, 1, 2]:
            if not pd.isna(df['dates'].iloc[i]):
                ...
                actual_date = pd.Timestamp(df_read['dates'].iloc[i]).strftime('%Y-%m-%d')
        ...
        for i in [0, 1, 3]:
            if not pd.isna(df['datetimes'].iloc[i]):
                expected_dt = pd.Timestamp(df['datetimes'].iloc[i]).strftime('%Y-%m-%d %H:%M')
                actual_dt = pd.Timestamp(df_read['datetimes'].iloc[i]).strftime('%Y-%m-%d %H:%M')
        ...
        for i in [0, 1, 3]:
            if not pd.isna(df['durations'].iloc[i]):
                ...
                assert abs(expected_sec - actual_sec) < 1  # Allow for small rounding errors
...
def test_write_compressed_vs_uncompressed():
    ...
    with tempfile.NamedTemporaryFile(suffix='.jmp', delete=False) as temp1, \
        ...
        for col in df_orig.columns:
            ...
            try:
                if pd.api.types.is_numeric_dtype(df_orig[col]):
                    ...
                    pd.testing.assert_series_equal(df_orig[col], df_read[col], 
                ...
                for i in range(len(df_orig)):
                    assert pd.isna(df_orig[col].iloc[i]) == pd.isna(df_read[col].iloc[i]), \
