...
def write_file_header(file: BinaryIO, df: pd.DataFrame, version: str) -> None:
    ...
    padding_size = OFFSET_NROWS - len(MAGIC_JMP)
...
def write_column_data(file: BinaryIO, column: pd.Series, offset: int, 
                     column_name: str, compress: bool = True) -> None:
    """
...
def get_column_data_type(column: pd.Series) -> str:
    ...
    if hasattr(dtype, 'name'):
        ...
        if dtype_name.startswith('datetime'):
            ...
            try:
                if all(pd.notna(t) and t.time() == time(0, 0) 
                       for t in pd.to_datetime(column.dropna())):
                    return 'date'
                return 'datetime'
